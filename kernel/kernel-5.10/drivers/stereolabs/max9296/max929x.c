...
int write_reg_Dser(int slaveAddr, int channel, u16 addr, u8 val)
{
    ...
    if (err)
    {
        dev_err(&i2c_client->dev, "%s: ERR : %d, i2c= 0x%x, addr = 0x%x, val = 0x%x\n",
                __func__, err, i2c_client->addr, addr, val);
        return -1;
    }
    return 0;
}
...
int links_check_Dser(int channel, u8 *links)
{
    ...
    err = read_reg_Dser(global_priv[channel]->max_address, channel, mode_table[tab_id][0].addr, &link);
...
static int max9296_read_reg(struct max9296 *priv, u16 addr,
        unsigned int *val)
{
...
static int max9296_write_table(struct max9296 *priv,
...
int fast_reset_Dser(int channel)
{
    ...
    err = max9296_write_table(global_priv[channel], mode_table[MAX9296_FST_RST]);
...
int slow_reset_Dser(int channel)
{
    ...
    err = max9296_write_table(global_priv[channel], mode_table[MAX9296_INIT]);
...
u32 fps_set_Dser(int channel, s64 val)
{
    ...
    err = max9296_write_table(global_priv[channel], mode_table[tab_id]);
...
static int max9296_stats_show(struct seq_file *s, void *data)
{
    return 0;
}

static int max9296_debugfs_open(struct inode *inode, struct file *file)
{
    return single_open(file, max9296_stats_show, inode->i_private);
}

static ssize_t max9296_debugfs_write(struct file *s,
...
static const struct file_operations max9296_debugfs_fops = {
    .open = max9296_debugfs_open,
    .read = seq_read,
    .write = max9296_debugfs_write,
    .llseek = seq_lseek,
    .release = single_release,
};

static int max9296_debugfs_init(const char *dir_name,
        ...
        struct max9296 *priv)
{
    ...
    if (np)
    {
        ...
        if (err)
            dev_err(&i2c_client->dev, "MAX address not found --> Requires 'reg' entry in DTS \n");

        err = snprintf(dev_name, sizeof(dev_name), "max9296_%s",
                str);
        if (err < 0)
            return -EINVAL;
    }
    ...
    fp = debugfs_create_file("max9296", S_IRUGO | S_IWUSR,
            dp, priv, &max9296_debugfs_fops);
    if (!fp)
    {
        ...
        const struct i2c_device_id *id)
{
    ...
    err = max9296_debugfs_init(NULL, &priv->dp, &priv->fp, priv);
    if (err)
        return err;
    ...
    static int
max9296_remove(struct i2c_client *client)
{
    int i;
    for (i = 0; i < 4; i++)
    {
        ...
        {
            dev_info(&client->dev, "%s: removing debugfs for max9296 %i\n", __func__ ,i);
