 ...
 *
 * @filename bmi2xy_i2c.c
 * @date	 14/05/2020
 * @version	 1.42.0
 *
 * @brief	 BMI2xy I2C bus Driver
 */
...
/* global variables */
/*********************************************************************/
static struct i2c_client *bmi2xy_i2c_client;
 ...
 * * negative value - Error.
 */
static s8 bmi2xy_i2c_read_wrapper(u8 reg_addr, u8 *data,
				u32 len, void *intf_ptr)
{
	s8 err;

	err = bmi2xy_i2c_read(bmi2xy_i2c_client, reg_addr, data, len);
	return err;
}
 ...
 * * negative value - Error.
 */
static s8 bmi2xy_i2c_write_wrapper(u8 reg_addr, const u8 *data,
				u32 len, void *intf_ptr)
{
	s8 err;

	err = bmi2xy_i2c_write(bmi2xy_i2c_client, reg_addr, data, len);
	return err;
}
				...
				const struct i2c_device_id *id)
{
	...
	if (bmi2xy_i2c_client == NULL) {
		bmi2xy_i2c_client = client;
	} else {
	...
	dev_id = BMI2_I2C_INTF;
	client_data->device.intf_ptr = &dev_id;
	client_data->device.intf = BMI2_I2C_INTF;
...
exit_err_clean:
	if (err)
		bmi2xy_i2c_client = NULL;
	return err;
}

/**
 *	bmi2xy_i2c_remove - Callback called when device is unbinded.
 ...
 *	* Negative value - Error.
 */
static int bmi2xy_i2c_remove(struct i2c_client *client)
{
	int err = 0;
...
static const struct i2c_device_id bmi2xy_id[] = {
	{ SENSOR_NAME, 0 },
	{ }
};

MODULE_DEVICE_TABLE(i2c, bmi2xy_id);
static const struct of_device_id bmi2xy_of_match[] = {
	{ .compatible = "bmi2xy", },
	{ }
};
MODULE_DEVICE_TABLE(i2c, bmi2xy_of_match);

static struct i2c_driver bmi2xy_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = SENSOR_NAME,
	...
	.id_table = bmi2xy_id,
	.probe = bmi2xy_i2c_probe,
	.remove = bmi2xy_i2c_remove,
};

/**
 *	bmi2xy_init - I2C driver init function.
 *
 *	Return : Status of the suspend function.
 *	* 0 - OK.
 *	* Negative value - Error.
 */
static int __init bmi2xy_init(void)
{
	PERR("bmi2xy_init");
	return i2c_add_driver(&bmi2xy_driver);
}

/**
 *	bmi2xy_exit - I2C driver exit function.
 */
static void __exit bmi2xy_exit(void)
{
	i2c_del_driver(&bmi2xy_driver);
}

MODULE_AUTHOR("contact@bosch-sensortec.com>");
MODULE_DESCRIPTION("BMI2XY SENSOR DRIVER");
MODULE_LICENSE("GPL v2");
/*lint -e19*/
module_init(bmi2xy_init);
module_exit(bmi2xy_exit);
/*lint +e19*/
